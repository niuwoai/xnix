...
assert(source.include?("desktop-trigger-request-preflight-preview"), "q4 D-Bus fixture smoke must require Go-owned desktop-trigger preflight")
assert(source.include?("desktop_trigger_preflight_checked"), "q4 D-Bus fixture smoke must report desktop-trigger preflight checks")
assert(source.include?("desktop_trigger_preflight_ready_for_operator_request"), "q4 D-Bus fixture smoke must report promoted operator-request readiness")
...
assert(payload.fetch("desktop_trigger_preflight_checked") == false, "plan must not claim desktop-trigger preflight before execute")
assert(payload.fetch("desktop_trigger_preflight_blocked_missing_promotion") == false, "plan must not claim fail-closed preflight before execute")
assert(payload.fetch("desktop_trigger_preflight_ready_for_operator_request") == false, "plan must not claim operator request readiness before execute")
assert(payload.fetch("desktop_trigger_preflight_owner_service_shape_verified") == false, "plan must not claim owner service shape verification before execute")
assert(payload.fetch("desktop_trigger_preflight_path_exposed") == false, "plan must not expose desktop-trigger preflight paths")
