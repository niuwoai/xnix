...
if go_available
  ...
  assert(kde_center_page_payload.fetch("source") == "compatibility-center-preview+backend-selection-preview+desktop-activation-status-preview+execution-readiness-preview+launch-intent-preview+kde-action-card-deck-preview+settings-preview", "KDE center page preview must compose existing Runtime read models")
  ...
  assert(kde_center_page_payload.fetch("navigation_count") == 8, "KDE center page must expose navigation sections")
  ...
  assert(kde_center_page_sections_payload.fetch("section_count") == 8, "KDE center page sections preview must expose eight sections")
  assert(kde_center_page_sections_payload.fetch("read_only_section_count") == 8, "KDE center page sections preview must mark all sections read-only")
  assert(kde_center_page_sections_payload.fetch("navigation_only_section_count") == 8, "KDE center page sections preview must mark all sections navigation-only")
  ...
  assert(kde_center_page_sections_payload.fetch("sections").map { |section| section.fetch("id") } == %w[overview backend activation execution launch actions settings diagnostics], "KDE center page sections preview must preserve page section order")
  assert(kde_center_page_sections_payload.fetch("sections").map { |section| section.fetch("runtime_method") } == %w[GetCompatibilityCenterSummary GetBackendSelectionPlan GetDesktopActivationStatus GetExecutionReadiness GetLaunchIntent GetCompatibilityActionQueue GetCompatibilitySettings GetDiagnostics], "KDE center page sections preview must bind sections to Runtime read methods")
  ...
  assert(kde_center_page_section_detail_payload.fetch("available_section_ids") == %w[overview backend activation execution launch actions settings diagnostics], "KDE center page section detail preview must expose available navigation")
