...
assert(dockerfile.include?("go test -timeout 90m ./..."), "Docker image must run Go tests")
...
if go_available
  ...
  assert(plan.fetch("launch_command") == ["xnix-compat-launch", "--app", recipe.id, "--registry", "/usr/share/xnix/compatibility/recipes/registry.json", "%U"], "plan must use the managed Runtime launcher")
  ...
  assert(center_payload.fetch("application_count") == 3, "Compatibility Center preview must summarize registered applications")
  ...
  center_app = center_payload.fetch("applications").find { |application| application.fetch("application_id") == recipe.id }
  assert(center_app, "Compatibility Center preview must include the target application")
  ...
  assert(readiness_application.fetch("launcher_command") == ["xnix-compat-launch", "--app", recipe.id, "--registry", "/usr/share/xnix/compatibility/recipes/registry.json", "%U"], "execution readiness preview must expose the managed launcher command")
  ...
  assert(launch_intent_payload.fetch("launcher_command") == ["xnix-compat-launch", "--app", recipe.id, "--registry", "/usr/share/xnix/compatibility/recipes/registry.json", "%U"], "launch intent preview must expose the managed launcher command")
  ...
  assert(execution_request_payload.fetch("launcher_command") == ["xnix-compat-launch", "--app", recipe.id, "--registry", "/usr/share/xnix/compatibility/recipes/registry.json", "%U"], "execution request preview must expose the managed launcher command")
  ...
  assert(kde_action_card_deck_payload.fetch("navigation_action_count") == 29, "KDE action card deck preview must expose navigation-only actions")
  ...
  execution_readiness_snapshot = kde_center_page_payload.fetch("execution_readiness_snapshot")
  assert(execution_readiness_snapshot.fetch("request_type") == "execution-readiness-preview", "KDE center page must include execution readiness snapshots")
  assert(execution_readiness_snapshot.fetch("runtime_method") == "GetExecutionReadiness", "KDE center page execution readiness must route to Runtime readiness reads")
  assert(execution_readiness_snapshot.fetch("execution_state") == "blocked", "KDE center page execution readiness must expose blocked state")
  assert(execution_readiness_snapshot.fetch("overall_status") == "not-ready", "KDE center page execution readiness must not claim launch readiness")
  assert(execution_readiness_snapshot.fetch("gate_count") == 5, "KDE center page execution readiness must expose gate counts")
  assert(execution_readiness_snapshot.fetch("desktop_entry_launch_visible") == true, "KDE center page execution readiness must keep normal desktop entries visible")
  assert(execution_readiness_snapshot.fetch("launch_allowed") == false, "KDE center page execution readiness must keep launch disallowed")
  assert(execution_readiness_snapshot.fetch("launch_enabled") == false, "KDE center page execution readiness must keep launch disabled")
  assert(execution_readiness_snapshot.fetch("execution_request_created") == false, "KDE center page execution readiness must not create execution requests")
  assert(execution_readiness_snapshot.fetch("backend_binding_ready") == false, "KDE center page execution readiness must not claim backend binding readiness")
  assert(execution_readiness_snapshot.fetch("host_root_modified") == false, "KDE center page execution readiness must not mutate the host root")
  assert(execution_readiness_snapshot.fetch("backend_details_exposed") == false, "KDE center page execution readiness must hide backend details")
  launch_intent_snapshot = kde_center_page_payload.fetch("launch_intent_snapshot")
  assert(launch_intent_snapshot.fetch("request_type") == "launch-intent-preview", "KDE center page must include launch intent snapshots")
  assert(launch_intent_snapshot.fetch("intent_type") == "runtime-launch-intent", "KDE center page launch intent must identify Runtime launch intents")
  assert(launch_intent_snapshot.fetch("source") == "desktop-launcher", "KDE center page launch intent must preserve desktop launcher source")
  assert(launch_intent_snapshot.fetch("runtime_method") == "Launch", "KDE center page launch intent must point at Runtime Launch")
  assert(launch_intent_snapshot.fetch("read_method") == "GetLaunchIntent", "KDE center page launch intent must expose the Runtime read method")
  assert(launch_intent_snapshot.fetch("file_count") == 1, "KDE center page launch intent must count selected files")
  assert(launch_intent_snapshot.fetch("portal_required") == true, "KDE center page launch intent must require Portal review for files")
  assert(launch_intent_snapshot.fetch("snapshot_required") == true, "KDE center page launch intent must require snapshots")
  assert(launch_intent_snapshot.fetch("standard_desktop_entry") == true, "KDE center page launch intent must preserve desktop entries")
  assert(launch_intent_snapshot.fetch("launch_uses_runtime") == true, "KDE center page launch intent must route through Runtime")
  assert(launch_intent_snapshot.fetch("launch_allowed") == false, "KDE center page launch intent must keep launch disallowed")
  assert(launch_intent_snapshot.fetch("launch_enabled") == false, "KDE center page launch intent must keep launch disabled")
  assert(launch_intent_snapshot.fetch("execution_request_created") == false, "KDE center page launch intent must not create execution requests")
  assert(launch_intent_snapshot.fetch("execution_started") == false, "KDE center page launch intent must not start execution")
  assert(launch_intent_snapshot.fetch("request_object_created") == false, "KDE center page launch intent must not create request objects")
  assert(launch_intent_snapshot.fetch("permission_granted") == false, "KDE center page launch intent must not grant permissions")
  assert(launch_intent_snapshot.fetch("host_root_modified") == false, "KDE center page launch intent must not mutate the host root")
  assert(launch_intent_snapshot.fetch("backend_details_exposed") == false, "KDE center page launch intent must hide backend details")
  window_identity_snapshot = kde_center_page_payload.fetch("window_identity_snapshot")
  assert(window_identity_snapshot.fetch("schema_version") == "xnix.runtime.window_identity.v1", "KDE center page must include window identity snapshots")
  assert(window_identity_snapshot.fetch("desktop_file") == "xnix-#{recipe.id}.desktop", "KDE center page window identity must bind generated desktop files")
  assert(window_identity_snapshot.fetch("launcher_url") == "applications:xnix-#{recipe.id}.desktop", "KDE center page window identity must expose launcher URLs")
  assert(window_identity_snapshot.fetch("window_kind") == "compatibility-application", "KDE center page window identity must keep compatibility window kinds")
  assert(window_identity_snapshot.fetch("class_group") == "xnix-compatibility", "KDE center page window identity must expose class groups")
  assert(window_identity_snapshot.fetch("resource_name") == recipe.id, "KDE center page window identity must expose resource names")
  assert(window_identity_snapshot.fetch("task_manager_grouping_key") == recipe.id, "KDE center page window identity must expose task-manager grouping")
  assert(window_identity_snapshot.fetch("task_manager_pinning_allowed") == true, "KDE center page window identity must allow pinning plans")
  assert(window_identity_snapshot.fetch("task_manager_restore_allowed") == true, "KDE center page window identity must allow restore plans")
  assert(window_identity_snapshot.fetch("task_manager_skip_taskbar") == false, "KDE center page window identity must keep taskbar visibility")
  assert(window_identity_snapshot.fetch("task_manager_show_in_switcher") == true, "KDE center page window identity must keep switcher visibility")
  assert(window_identity_snapshot.fetch("prefer_existing_window") == true, "KDE center page window identity must prefer existing windows")
  assert(window_identity_snapshot.fetch("kwin_script_role") == "identity-and-layout", "KDE center page window identity must expose KWin rule roles")
  assert(window_identity_snapshot.fetch("kwin_placement") == "normal-window", "KDE center page window identity must expose placement hints")
  assert(window_identity_snapshot.fetch("window_manager_policy_only") == true, "KDE center page window identity must keep window-manager policy-only")
  assert(window_identity_snapshot.fetch("runtime_owns_backend_policy") == true, "KDE center page window identity must keep backend policy Runtime-owned")
  assert(window_identity_snapshot.fetch("task_manager_entry_active") == false, "KDE center page window identity must not activate task manager entries")
  assert(window_identity_snapshot.fetch("kwin_rule_applied") == false, "KDE center page window identity must not apply KWin rules")
  assert(window_identity_snapshot.fetch("host_root_modified") == false, "KDE center page window identity must not mutate the host root")
  assert(window_identity_snapshot.fetch("backend_details_exposed") == false, "KDE center page window identity must hide backend details")
  file_association_snapshot = kde_center_page_payload.fetch("file_association_snapshot")
  assert(file_association_snapshot.fetch("plan_type") == "file-association-plan", "KDE center page must include file association snapshots")
  assert(file_association_snapshot.fetch("runtime_method") == "GetFileAssociationPlan", "KDE center page file associations must route to Runtime file association reads")
  assert(file_association_snapshot.fetch("desktop_file") == "xnix-#{recipe.id}.desktop", "KDE center page file associations must bind generated desktop files")
  assert(file_association_snapshot.fetch("mimeapps_path") == "usr/share/applications/mimeapps.list", "KDE center page file associations must expose standard MIME paths")
  assert(file_association_snapshot.fetch("mime_type_count") == 2, "KDE center page file associations must expose MIME type counts")
  assert(file_association_snapshot.fetch("mime_types") == recipe.mime_types.sort, "KDE center page file associations must expose normalized MIME types")
  assert(file_association_snapshot.fetch("file_open_command") == "xnix-compat-open", "KDE center page file associations must expose managed file-open commands")
  assert(file_association_snapshot.fetch("file_open_argument") == "%U", "KDE center page file associations must keep file-open arguments abstract")
  assert(file_association_snapshot.fetch("standard_mimeapps_list") == true, "KDE center page file associations must use standard MIME apps lists")
  assert(file_association_snapshot.fetch("staged_root_only") == true, "KDE center page file associations must stay staged")
  assert(file_association_snapshot.fetch("overwrite_existing_mimeapps") == false, "KDE center page file associations must not overwrite MIME defaults")
  assert(file_association_snapshot.fetch("portal_required_for_file_open") == true, "KDE center page file associations must require Portal file-open routing")
  assert(file_association_snapshot.fetch("file_association_ready") == true, "KDE center page file associations must expose readiness")
  assert(file_association_snapshot.fetch("file_open_preview_available") == true, "KDE center page file associations must expose file-open previews")
  assert(file_association_snapshot.fetch("direct_host_file_access") == false, "KDE center page file associations must not allow direct host file access")
  assert(file_association_snapshot.fetch("request_object_created") == false, "KDE center page file associations must not create request objects")
  assert(file_association_snapshot.fetch("permission_granted") == false, "KDE center page file associations must not grant permissions")
  assert(file_association_snapshot.fetch("files_written") == false, "KDE center page file associations must not write files")
  assert(file_association_snapshot.fetch("mimeapps_written") == false, "KDE center page file associations must not write MIME apps lists")
  assert(file_association_snapshot.fetch("host_root_modified") == false, "KDE center page file associations must not mutate the host root")
  assert(file_association_snapshot.fetch("backend_details_exposed") == false, "KDE center page file associations must hide backend details")
  tray_status_snapshot = kde_center_page_payload.fetch("tray_status_snapshot")
  assert(tray_status_snapshot.fetch("status_type") == "tray-status-preview", "KDE center page must include tray status snapshots")
  assert(tray_status_snapshot.fetch("runtime_method") == "GetTrayStatus", "KDE center page tray status must route to Runtime tray reads")
  assert(tray_status_snapshot.fetch("desktop_file") == "xnix-#{recipe.id}.desktop", "KDE center page tray status must bind generated desktop files")
  assert(tray_status_snapshot.fetch("registered_application_count") == 1, "KDE center page tray status must expose registered application counts")
  assert(tray_status_snapshot.fetch("active_application_count") == 0, "KDE center page tray status must not claim live active applications")
  assert(tray_status_snapshot.fetch("attention_required_count") == 0, "KDE center page tray status must not claim live attention requests")
  assert(tray_status_snapshot.fetch("compatibility_state") == "ready", "KDE center page tray status must expose compatibility state")
  assert(tray_status_snapshot.fetch("tray_bridge_state") == "planned", "KDE center page tray status must expose bridge state")
  assert(tray_status_snapshot.fetch("bridged_tray_application_count") == 0, "KDE center page tray status must not expose live bridged tray apps")
  assert(tray_status_snapshot.fetch("actions") == %w[open-compatibility-center open-settings], "KDE center page tray status must expose safe navigation actions")
  assert(tray_status_snapshot.fetch("action_count") == 2, "KDE center page tray status must count safe actions")
  assert(tray_status_snapshot.fetch("user_visible") == true, "KDE center page tray status must be user visible")
  assert(tray_status_snapshot.fetch("tray_status_ready") == true, "KDE center page tray status must expose readiness")
  assert(tray_status_snapshot.fetch("live_backend_bridge_enabled") == false, "KDE center page tray status must not enable live backend tray bridging")
  assert(tray_status_snapshot.fetch("bridge_configuration_persisted") == false, "KDE center page tray status must not persist bridge configuration")
  assert(tray_status_snapshot.fetch("host_root_modified") == false, "KDE center page tray status must not mutate the host root")
  assert(tray_status_snapshot.fetch("backend_details_exposed") == false, "KDE center page tray status must hide backend details")
  notification_snapshot = kde_center_page_payload.fetch("notification_snapshot")
  assert(notification_snapshot.fetch("request_type") == "desktop-notification-preview", "KDE center page must include notification snapshots")
  assert(notification_snapshot.fetch("runtime_method") == "GetNotificationPlan", "KDE center page notifications must route to Runtime notification reads")
  assert(notification_snapshot.fetch("source") == "runtime-event", "KDE center page notifications must originate from Runtime events")
  assert(notification_snapshot.fetch("desktop_file") == "xnix-#{recipe.id}.desktop", "KDE center page notifications must bind generated desktop files")
  assert(notification_snapshot.fetch("event_type") == "approval-required", "KDE center page notifications must expose approval events")
  assert(notification_snapshot.fetch("notification_id") == "#{recipe.id}.approval-required", "KDE center page notifications must expose stable notification ids")
  assert(notification_snapshot.fetch("urgency") == "critical", "KDE center page notifications must expose urgency")
  assert(notification_snapshot.fetch("category") == "compatibility.approval", "KDE center page notifications must expose categories")
  assert(notification_snapshot.fetch("action_count") == 2, "KDE center page notifications must expose action counts")
  assert(notification_snapshot.fetch("actions") == %w[open-compatibility-center review-request], "KDE center page notifications must expose safe notification actions")
  assert(notification_snapshot.fetch("requires_user_review") == true, "KDE center page notifications must expose review requirements")
  assert(notification_snapshot.fetch("user_visible") == true, "KDE center page notifications must be user visible")
  assert(notification_snapshot.fetch("notification_ready") == true, "KDE center page notifications must expose readiness")
  assert(notification_snapshot.fetch("action_execution_enabled") == false, "KDE center page notifications must not enable actions")
  assert(notification_snapshot.fetch("repair_execution_enabled") == false, "KDE center page notifications must not enable repair execution")
  assert(notification_snapshot.fetch("settings_persistence_enabled") == false, "KDE center page notifications must not enable settings persistence")
  assert(notification_snapshot.fetch("notifications_sent") == false, "KDE center page notifications must not send notifications")
  assert(notification_snapshot.fetch("host_root_modified") == false, "KDE center page notifications must not mutate the host root")
  assert(notification_snapshot.fetch("backend_details_exposed") == false, "KDE center page notifications must hide backend details")
  assert(kde_center_page_payload.fetch("navigation_count") == 12, "KDE center page must expose navigation sections")
  ...
  assert(kde_center_page_sections_payload.fetch("section_count") == 12, "KDE center page sections preview must expose twelve sections")
  assert(kde_center_page_sections_payload.fetch("read_only_section_count") == 12, "KDE center page sections preview must mark all sections read-only")
  assert(kde_center_page_sections_payload.fetch("navigation_only_section_count") == 12, "KDE center page sections preview must mark all sections navigation-only")
  ...
  assert(kde_center_page_sections_payload.fetch("sections").map { |section| section.fetch("id") } == %w[overview backend activation execution launch window files tray notifications actions settings diagnostics], "KDE center page sections preview must preserve page section order")
  assert(kde_center_page_sections_payload.fetch("sections").map { |section| section.fetch("runtime_method") } == %w[GetCompatibilityCenterSummary GetBackendSelectionPlan GetDesktopActivationStatus GetExecutionReadiness GetLaunchIntent GetTaskManagerIdentityPlan GetFileAssociationPlan GetTrayStatus GetNotificationPlan GetCompatibilityActionQueue GetCompatibilitySettings GetDiagnostics], "KDE center page sections preview must bind sections to Runtime read methods")
  assert(kde_center_page_sections_payload.fetch("sections").map { |section| section.fetch("read_model") }.last == "diagnostic-history-preview", "KDE center diagnostics section must route to the diagnostic history read model")
  ...
  assert(kde_center_page_section_detail_payload.fetch("available_section_ids") == %w[overview backend activation execution launch window files tray notifications actions settings diagnostics], "KDE center page section detail preview must expose available navigation")
  ...
  assert(krunner_match.fetch("action").fetch("argv") == ["xnix-compat-launch", "--app", recipe.id, "--registry", "/usr/share/xnix/compatibility/recipes/registry.json"], "KRunner preview must delegate to the managed launcher")
