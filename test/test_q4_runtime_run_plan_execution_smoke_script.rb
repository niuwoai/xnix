...
assert(source.include?("known_app_gui_review_action_present"), "q4 Runtime run-plan execution smoke must keep app-execution GUI evidence review-only")
...
assert(payload.fetch("known_app_gui_review_action_present") == false, "plan must not claim review action evidence before execute")
