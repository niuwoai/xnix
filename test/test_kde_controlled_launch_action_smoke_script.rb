...
assert(contents.include?("known_app_owner_file_open_entrypoint_count"), "KDE action smoke must verify owner file-open entrypoint evidence count")
assert(contents.include?("owner_file_open_verified"), "KDE action smoke must verify owner file-open action evidence")
assert(contents.include?("owner_file_open_entrypoint_invoked"), "KDE action smoke must verify owner file-open entrypoint action evidence")
assert(contents.include?("static_file_open_entrypoint"), "KDE action smoke must verify the static file-open entrypoint")
assert(contents.include?("owner_file_open_environment_ready"), "KDE action smoke must verify owner file-open environment readiness")
assert(contents.include?("owner_file_open_environment_keys"), "KDE action smoke must verify owner file-open environment keys")
assert(contents.include?("owner_file_open_environment_values_exposed"), "KDE action smoke must verify owner file-open environment values stay hidden")
...
assert(contents.include?("XNIX_ALLOW_LOCAL_GO_COMPILE"), "KDE action smoke must keep local Go compilation behind an explicit override")
assert(contents.include?("scripts/remote_go_build.rb --execute"), "KDE action smoke skip guidance must point compile-heavy work to q4")
