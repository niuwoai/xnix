...
assert(source.include?("--remote-import-record"), "q4 staged external smoke must accept existing Runtime import records")
assert(source.include?("existing_import_record_consumed"), "q4 staged external smoke must report existing import record consumption")
assert(source.include?("record_first_launch_path"), "q4 staged external smoke must expose record-first launch evidence")
assert(source.include?("windows_process_file_argument_window_observed"), "q4 staged external smoke must preserve Windows file-argument window evidence")
...
assert(source.include?("compatibility_evidence_bundle_path"), "q4 staged external smoke must expose the q4 compatibility evidence bundle path for follow-up Runtime commands")
...
assert(plan.fetch("desktop_argument_mode") == "file-uri", "plan must default to the file-open desktop argument lane")
assert(plan.fetch("desktop_file_open_lane") == true, "plan must identify the default file-open desktop lane")
...
assert(plan.fetch("artifact_output_count") == 13, "plan must announce all desktop evidence artifacts")
