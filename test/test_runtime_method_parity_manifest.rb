...
assert(manifest["version"] == File.read(File.expand_path("../VERSION", __dir__)).strip, "Runtime method parity manifest must expose the current version")
...
assert(manifest["method_count"] == 61, "Runtime method parity manifest must count read-only methods")
