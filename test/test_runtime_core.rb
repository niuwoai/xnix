...
assert(core_header.read.include?("#define XNIX_RUNTIME_VERSION \"0.2.58\""), "C Runtime core must expose the current version")
...
Dir.mktmpdir("xnix-runtime-core") do |dir|
  ...
  assert(probe["version"] == "0.2.58", "C Runtime core probe must expose the current version")
  ...
  assert(engine_catalog["version"] == "0.2.58", "C Runtime core engine catalog must expose the current version")
