...
assert(command.fetch(command.index("-cpu") + 1) == Xnix::Qemu::CPU_MODEL, "QEMU command must use the default CPU model")
...
assert(wine_command.fetch(wine_command.index("-cpu") + 1) == Xnix::Qemu::WINE_CPU_MODEL, "Wine guest QEMU command must use the i386-safe Wine CPU model")
