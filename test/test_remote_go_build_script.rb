...
assert(payload["package_count"] == 3, "remote Go build must build three core command packages by default")
assert(payload["build_targets"].map { |entry| entry.fetch("package") } == %w[./cmd/xnix-runtime-go ./cmd/xnix-runtime-owner ./cmd/xnix-compat-launch], "remote Go build must expose default package order")
