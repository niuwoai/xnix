...
assert(contents.include?("--app"), "D-Bus controlled launch owner fixture smoke must allow the known app id to be selected")
assert(contents.include?("--gui-smoke-evidence-file"), "D-Bus controlled launch owner fixture smoke must accept app-execution GUI evidence")
assert(contents.include?("--runtime-command"), "D-Bus controlled launch owner fixture smoke must accept a prebuilt Runtime command")
assert(contents.include?("XNIX_ALLOW_LOCAL_GO_COMPILE"), "D-Bus controlled launch owner fixture smoke must guard local Go compilation")
assert(contents.include?("ORIGINAL_ARGV"), "D-Bus controlled launch owner fixture smoke must preserve arguments across dbus-run-session re-entry")
assert(contents.include?("delegated_payload"), "D-Bus controlled launch owner fixture smoke must derive fake delegated payload ids from the Go fixture")
assert(contents.include?("desktop-trigger-service-call-materialization-preview"), "D-Bus controlled launch owner fixture smoke must consume the Go-owned service call materialization preview")
assert(contents.include?("PreviewDesktopTriggerServiceCallMaterialization"), "D-Bus controlled launch owner fixture smoke must verify the Go materialization preview runtime method")
assert(contents.include?("GetDesktopTriggerServiceCallMaterialization"), "D-Bus controlled launch owner fixture smoke must verify the Go materialization preview read method")
assert(contents.include?("--human-authorized-smoke"), "D-Bus controlled launch owner fixture smoke must explicitly request a human-authorized smoke candidate")
...
assert(contents.include?("go_service_call_materialization_json"), "D-Bus controlled launch owner fixture smoke must parse the nested Go service call materialization payload")
assert(contents.include?("assert_service_call_materialization_response"), "D-Bus controlled launch owner fixture smoke must verify the Go service call materialization response")
...
assert(contents.include?("owner_trigger_state"), "D-Bus controlled launch owner fixture smoke must verify materialized owner trigger readiness")
...
assert(contents.include?("service_call_materialization_from_runtime"), "D-Bus controlled launch owner fixture smoke must consume the Go-owned service call materialization preview")
