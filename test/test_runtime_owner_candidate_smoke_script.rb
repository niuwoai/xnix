...
assert(contents.include?("service-call"), "Runtime owner candidate smoke must verify owner service calls")
assert(contents.include?("xnix.runtime.owner_service_call.v1"), "Runtime owner candidate smoke must validate the owner service-call schema")
assert(contents.include?("go-runtime-owner-in-process-service"), "Runtime owner candidate smoke must validate the in-process owner service boundary")
assert(contents.include?("session-bus-smoke"), "Runtime owner candidate smoke must verify private session-bus owner smoke")
assert(contents.include?("xnix.runtime.owner_session_bus_smoke.v1"), "Runtime owner candidate smoke must validate the private session-bus owner smoke schema")
assert(contents.include?("runtime-owner-session-bus-smoke-step"), "Runtime owner candidate smoke must validate private session-bus owner smoke steps")
assert(contents.include?("restricted-private-session-bus-owner-smoke"), "Runtime owner candidate smoke must validate restricted private session-bus evidence")
assert(contents.include?("org.xnix.Compatibility1.Error.UnsupportedMethod"), "Runtime owner candidate smoke must validate unsupported-read failures")
