...
assert(kde_center_page["backend_selection_runtime_method"] == "GetBackendSelectionPlan", "dispatch must expose KDE center backend selection Runtime methods")
assert(kde_center_page["backend_recommended_profile_id"] == "local-compatibility", "dispatch must expose KDE center backend recommendations")
assert(!kde_center_page["backend_selection_committed"], "dispatch must keep KDE center backend selection uncommitted")
assert(!kde_center_page["backend_selection_change_enabled"], "dispatch must keep KDE center backend selection changes disabled")
assert(!kde_center_page["backend_launch_enabled"], "dispatch must keep KDE center backend launch disabled")
...
assert(kde_center_page["execution_readiness_runtime_method"] == "GetExecutionReadiness", "dispatch must expose KDE center execution readiness Runtime methods")
assert(kde_center_page["execution_state"] == "blocked", "dispatch must expose KDE center execution state")
assert(kde_center_page["execution_overall_status"] == "not-ready", "dispatch must expose KDE center execution readiness status")
assert(!kde_center_page["execution_launch_allowed"], "dispatch must keep KDE center execution launch disallowed")
assert(!kde_center_page["execution_launch_enabled"], "dispatch must keep KDE center execution launch disabled")
assert(!kde_center_page["execution_request_created"], "dispatch must not create KDE center execution requests")
assert(!kde_center_page["execution_backend_binding_ready"], "dispatch must not claim KDE center execution backend binding readiness")
assert(kde_center_page["desktop_entry_launch_visible"], "dispatch must keep desktop entries visible while launch is gated")
assert(kde_center_page["launch_intent_type"] == "runtime-launch-intent", "dispatch must expose KDE center launch intent snapshots")
assert(kde_center_page["launch_intent_runtime_method"] == "Launch", "dispatch must expose KDE center launch intent Runtime methods")
assert(kde_center_page["launch_intent_read_method"] == "GetLaunchIntent", "dispatch must expose KDE center launch intent read methods")
assert(!kde_center_page["launch_intent_allowed"], "dispatch must keep KDE center launch intent disallowed")
assert(!kde_center_page["launch_intent_enabled"], "dispatch must keep KDE center launch intent disabled")
assert(!kde_center_page["launch_intent_request_created"], "dispatch must not create KDE center launch intent requests")
assert(kde_center_page["window_identity_runtime_method"] == "GetTaskManagerIdentityPlan", "dispatch must expose KDE center window identity Runtime methods")
assert(kde_center_page["window_identity_desktop_file"] == "xnix-org.xnix.sample.notepad.desktop", "dispatch must expose KDE center window desktop files")
assert(kde_center_page["window_identity_grouping_key"] == "org.xnix.sample.notepad", "dispatch must expose KDE center task-manager grouping")
assert(kde_center_page["window_identity_pinning_allowed"], "dispatch must expose KDE center task-manager pinning")
assert(kde_center_page["window_identity_restore_allowed"], "dispatch must expose KDE center restore allowance")
assert(!kde_center_page["window_identity_skip_taskbar"], "dispatch must keep KDE center taskbar visibility")
assert(kde_center_page["window_identity_show_in_switcher"], "dispatch must keep KDE center switcher visibility")
assert(kde_center_page["window_identity_policy_only"], "dispatch must keep KDE center window identity policy-only")
assert(!kde_center_page["window_identity_task_manager_active"], "dispatch must not activate task manager entries")
assert(!kde_center_page["window_identity_kwin_rule_applied"], "dispatch must not apply KWin rules")
assert(kde_center_page["file_association_runtime_method"] == "GetFileAssociationPlan", "dispatch must expose KDE center file association Runtime methods")
assert(kde_center_page["file_association_plan_type"] == "file-association-plan", "dispatch must expose KDE center file association plan types")
assert(kde_center_page["file_association_desktop_file"] == "xnix-org.xnix.sample.notepad.desktop", "dispatch must expose KDE center file association desktop files")
assert(kde_center_page["file_association_count"] == 2, "dispatch must expose KDE center file association counts")
assert(kde_center_page["file_association_standard_mimeapps"], "dispatch must expose standard KDE center MIME associations")
assert(kde_center_page["file_association_staged_root_only"], "dispatch must keep KDE center file association staging scoped")
assert(!kde_center_page["file_association_overwrite_existing"], "dispatch must not overwrite existing MIME defaults")
assert(kde_center_page["file_association_portal_required"], "dispatch must require Portal file-open routing")
assert(!kde_center_page["file_association_direct_host_file_access"], "dispatch must not allow direct host file access")
assert(!kde_center_page["file_association_request_object_created"], "dispatch must not create file-open request objects")
assert(!kde_center_page["file_association_permission_granted"], "dispatch must not grant file permissions")
assert(!kde_center_page["file_association_files_written"], "dispatch must not write file associations")
assert(kde_center_page["tray_status_runtime_method"] == "GetTrayStatus", "dispatch must expose KDE center tray Runtime methods")
assert(kde_center_page["tray_status_type"] == "tray-status-plan", "dispatch must expose KDE center tray status types")
assert(kde_center_page["tray_status_active_application_count"] == 1, "dispatch must expose active tray application counts")
assert(kde_center_page["tray_status_attention_required_count"] == 1, "dispatch must expose tray attention counts")
assert(kde_center_page["tray_status_registered_application_count"] == 1, "dispatch must expose registered tray application counts")
assert(kde_center_page["tray_status_compatibility_state"] == "attention-required", "dispatch must expose tray compatibility state")
assert(kde_center_page["tray_status_bridge_state"] == "idle", "dispatch must expose tray bridge state")
assert(kde_center_page["tray_status_bridged_application_count"].zero?, "dispatch must not expose live bridged tray applications")
assert(!kde_center_page["tray_status_live_bridge_enabled"], "dispatch must keep live tray bridge disabled")
assert(!kde_center_page["tray_status_bridge_configuration_persisted"], "dispatch must not persist tray bridge configuration")
assert(kde_center_page["tray_status_ready"], "dispatch must mark tray status ready")
assert(kde_center_page["notification_runtime_method"] == "GetNotificationPlan", "dispatch must expose KDE center notification Runtime methods")
assert(kde_center_page["notification_plan_type"] == "notification-plan", "dispatch must expose notification plan types")
assert(kde_center_page["notification_event_type"] == "approval-required", "dispatch must expose notification events")
assert(kde_center_page["notification_id"] == "org.xnix.sample.notepad.approval-required", "dispatch must expose notification ids")
assert(kde_center_page["notification_urgency"] == "critical", "dispatch must expose notification urgency")
assert(kde_center_page["notification_category"] == "compatibility.approval.required", "dispatch must expose notification categories")
assert(kde_center_page["notification_action_count"] == 2, "dispatch must expose notification action counts")
assert(kde_center_page["notification_requires_user_review"], "dispatch must expose notification review requirements")
assert(!kde_center_page["notification_action_execution_enabled"], "dispatch must keep notification actions gated")
assert(!kde_center_page["notification_repair_execution_enabled"], "dispatch must keep notification repair gated")
assert(!kde_center_page["notification_settings_persistence_enabled"], "dispatch must keep notification settings persistence gated")
assert(!kde_center_page["notification_sent"], "dispatch must not send notifications from page previews")
assert(kde_center_page["notification_ready"], "dispatch must mark notification ready")
...
assert(kde_center_page_sections["section_count"] == 12, "dispatch must expose KDE center page section counts")
assert(kde_center_page_sections["read_only_section_count"] == 12, "dispatch must keep KDE center page sections read-only")
...
assert(kde_center_page_section_detail["available_section_ids"] == %w[overview backend activation execution launch window files tray notifications actions settings diagnostics], "dispatch must expose selected section navigation")
