...
assert(kde_center_page_sections["section_count"] == 8, "dispatch must expose KDE center page section counts")
assert(kde_center_page_sections["read_only_section_count"] == 8, "dispatch must keep KDE center page sections read-only")
...
assert(kde_center_page_sections["runtime_methods"] == %w[GetCompatibilityCenterSummary GetBackendSelectionPlan GetDesktopActivationStatus GetExecutionReadiness GetLaunchIntent GetCompatibilityActionQueue GetCompatibilitySettings GetDiagnostics], "dispatch must expose KDE center page section read methods")
...
assert(kde_center_page_section_detail["available_section_ids"] == %w[overview backend activation execution launch actions settings diagnostics], "dispatch must expose selected section navigation")
