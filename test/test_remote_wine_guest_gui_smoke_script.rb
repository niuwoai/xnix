...
assert(script.read.include?("file_open_static_entrypoint"), "remote GUI smoke execute result must expose the static file-open entrypoint")
assert(script.read.include?("owner_file_open_environment_ready"), "remote GUI smoke execute result must expose owner file-open environment readiness")
assert(script.read.include?("owner_file_open_environment_keys"), "remote GUI smoke execute result must expose safe owner file-open environment key names")
assert(script.read.include?("owner_file_open_environment_values_exposed"), "remote GUI smoke execute result must prove owner file-open environment values stay hidden")
assert(script.read.include?("kde_action_owner_file_open_environment_ready"), "remote GUI smoke execute result must expose KDE action owner file-open environment readiness")
assert(script.read.include?("kde_action_owner_file_open_environment_keys"), "remote GUI smoke execute result must expose KDE action safe owner file-open environment key names")
...
assert(file_open_owner_payload["file_open_static_entrypoint"] == "xnix-compat-open %U", "remote GUI smoke file-open owner plan must expose the static file-open entrypoint")
assert(file_open_owner_payload["owner_file_open_environment_ready"] == true, "remote GUI smoke file-open owner plan must mark owner file-open environment readiness")
assert(file_open_owner_payload["owner_file_open_environment_key_count"] == file_open_owner_payload.fetch("owner_file_open_environment_keys").length, "remote GUI smoke file-open owner plan must count owner file-open environment keys")
%w[XNIX_COMPAT_LAUNCH XNIX_COMPAT_OPEN_STATE_ROOT XNIX_COMPAT_OPEN_RECEIPT_ID XNIX_COMPAT_OPEN_REVIEW_RECEIPT_ID XNIX_COMPAT_OPEN_GUEST_HOST XNIX_COMPAT_OPEN_WINDOW_MATCH].each do |key|
  assert(file_open_owner_payload.fetch("owner_file_open_environment_keys").include?(key), "remote GUI smoke file-open owner plan must expose #{key} as a safe key name")
end
