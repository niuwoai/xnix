...
assert(external_app_smoke_index < fixture_app_smoke_index, "full smoke must prove imported external Windows app desktop smoke before fixture regression smoke")
...
assert(script.include?("PASS: staged desktop external Windows app smoke"), "full smoke must require staged external Windows app desktop PASS evidence")
...
assert(script.include?("Xnix::FullSmokeReport.classify_failure"), "full smoke must classify failed steps before aborting")
assert(script.include?("fail_full_smoke"), "full smoke must write failure reports before aborting")
assert(script.include?("write_report"), "full smoke must share successful and failed report writing")
