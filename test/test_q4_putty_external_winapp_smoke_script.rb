...
assert(source.include?("--desktop-argument-mode"), "q4 PuTTY smoke must select a desktop argument mode")
assert(source.include?("\"none\""), "q4 PuTTY smoke must use the no-file-argument desktop launch lane")
...
assert(payload.fetch("desktop_argument_mode") == "none", "plan must use the argumentless desktop launch lane")
assert(payload.fetch("desktop_file_open_lane") == false, "plan must not claim file-open behavior for PuTTY")
