...
Dir.mktmpdir("xnix-desktop-root") do |root|
  ...
  assert(result["version"] == File.read(File.expand_path("../VERSION", __dir__)).strip, "desktop activation installer must expose the current version")
  ...
  assert(result["installed"].length == 5, "desktop activation installer must install five staged files")
  ...
  assert(launcher_artifact_path.file?, "desktop activation installer must stage the managed launcher artifact receipt")
  launcher_artifact = JSON.parse(launcher_artifact_path.read)
  assert(launcher_artifact["command"] == "xnix-compat-launch", "managed launcher artifact must name the desktop command")
  assert(launcher_artifact["source_package"] == "cmd/xnix-compat-launch", "managed launcher artifact must point to the Go source package")
  assert(launcher_artifact["build_output"] == "usr/local/bin/xnix-compat-launch", "managed launcher artifact must point to the packaged Go launcher")
  assert(launcher_artifact["runtime_method"] == "PreviewKnownPortableLaunchBridge", "managed launcher artifact must route through the Runtime bridge")
  assert(launcher_artifact["dispatch_gate"] == "managed-known-app-guest-smoke", "managed launcher artifact must preserve the dispatch gate")
  assert(launcher_artifact["desktop_exec_uses_command"], "managed launcher artifact must be tied to the desktop Exec command")
  assert(!launcher_artifact["binary_copied"], "managed launcher artifact staging must not copy binaries")
  assert(launcher_artifact["runtime_owned"], "managed launcher artifact must stay Runtime-owned")
  assert(launcher_artifact["go_runtime_backed"], "managed launcher artifact must be Go-backed")
  assert(!launcher_artifact["host_root_modified"], "managed launcher artifact must not mutate the host root")
  assert(!launcher_artifact["backend_details_exposed"], "managed launcher artifact must not expose backend details")
  assert(!launcher_artifact_path.read.match?(/wine|prefix|\.exe|proton|qemu-system|program files/i), "managed launcher artifact must not expose backend implementation terms")
