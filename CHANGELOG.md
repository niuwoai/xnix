...
- Added catalog-derived q4 known portable run plan fields for app version, artifact kind, download archive name, executable relative path, and supported q4 portable-bundle app IDs.
- Added Runtime and CLI tests proving `q4-known-portable-winapp-run-plan-preview` consumes the Go Runtime known portable catalog and rejects known catalog apps outside the portable-bundle q4 lane.

### Changed

- Updated the q4 targeted `remote_go_test.rb` default regex and layout checks to cover the catalog-backed known portable run plan.
- Updated README, product overview, Claude guide, Runtime metadata, KDE metadata, known Windows GUI app catalog entries, and development recipe versions to `0.2.640-rc272`.
...
- Added the Go Runtime `external-winapp-import-stage-and-launch` command so an imported external `.exe` can be staged for KDE desktop activation and then launched through the staged or explicitly supplied `xnix-compat-launch`.

### Changed

- Updated Runtime, KDE metadata, known Windows GUI app catalog entries, and development recipe versions to `0.2.640-rc232`.
...
- Added the Go Runtime `external-winapp-import-and-run` command so a local `.exe` can be imported into Runtime state and immediately run through the isolated Wine/Xvfb backend with optional KDE-style file URI arguments.

### Changed

- Updated Runtime, KDE metadata, known Windows GUI app catalog entries, and development recipe versions to `0.2.640-rc230`.
