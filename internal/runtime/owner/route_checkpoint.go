...
func NewRouteCheckpoint(root string) (RouteCheckpoint, error) {
	...
	ownerLocalCoverageReady := ownerLocalCount == 6
	...
	checks := []RouteCheckpointCheck{
		...
		checkpointCheck("owner-local-routes", ownerLocalCoverageReady, "Six review-only owner-local routes are present outside the production ABI."),
