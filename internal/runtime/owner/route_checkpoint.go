...
func NewRouteCheckpoint(root string) (RouteCheckpoint, error) {
	...
	ownerLocalCoverageReady := ownerLocalCount == 5
	...
	checks := []RouteCheckpointCheck{
		...
		checkpointCheck("owner-local-routes", ownerLocalCoverageReady, "Five review-only owner-local routes are present outside the production ABI."),
