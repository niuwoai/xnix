...
func NewRouteCheckpoint(root string) (RouteCheckpoint, error) {
	...
	ownerLocalCoverageReady := ownerLocalCount == 4
	...
	checks := []RouteCheckpointCheck{
		...
		checkpointCheck("owner-local-routes", ownerLocalCoverageReady, "Four review-only owner-local routes are present outside the production ABI."),
