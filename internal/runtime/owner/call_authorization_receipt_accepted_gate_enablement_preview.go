...
func acceptedReceiptGateEnablementItems(grantPreview , mainlineReady, grantReady bool) []map[string]any {
	...
	for _, sourceItem := range sourceItems {
		itemGrantAuthorizationAcceptanceCallAuthorizationCallPersistenceAuthorizationEvidenceReady := grantReady && sourceItem["persistence_receipt_consumer_enablement_receipt_storage_record_writer_call_authorization_receipt_accepted_receipt_gate_authorization_acceptance_call_authorization_call_persistence_authorization_evidence_ready"] == true
		itemReady := ready && itemGrantAuthorizationAcceptanceCallAuthorizationCallPersistenceAuthorizationEvidenceReady
		itemStatus := status
		if !itemReady {
