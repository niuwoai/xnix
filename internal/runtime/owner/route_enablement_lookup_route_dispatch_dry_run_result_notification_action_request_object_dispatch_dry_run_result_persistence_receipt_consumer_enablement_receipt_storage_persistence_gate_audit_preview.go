...
func (root string) (, error) {
	...
	items := routeEnablementLookupRouteDispatchDryRunResultNotificationActionRequestObjectDispatchDryRunResultPersistenceReceiptConsumerEnablementReceiptStoragePersistenceGateAuditItems(mainlineReady, closedPersistenceImplementationReady)
	...
	ready := mainlineReady && closedPersistenceImplementationReady && readyItemCount == len(items)
...
func routeEnablementLookupRouteDispatchDryRunResultNotificationActionRequestObjectDispatchDryRunResultPersistenceReceiptConsumerEnablementReceiptStoragePersistenceGateAuditItems(mainlineReady, closedPersistenceImplementationReady bool) []map[string]any {
	kinds := []string{"install-failure", "repair-suggestion", "environment-switch", "approval-info"}
	items := make([]map[string]any, 0, len(kinds))
	for _, kind := range kinds {
		ready := mainlineReady && closedPersistenceImplementationReady
