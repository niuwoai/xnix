...
func NewProductionRollbackDiagnosticsReviewPreview(root string) (ProductionRollbackDiagnosticsReviewPreview, error) {
	...
	preview := ProductionRollbackDiagnosticsReviewPreview{
		...
		Source:                   "production-dbus-gate-review-preview+production-dbus-method-review-preview+runtime-service-activation-preflight-preview+production-human-authorization-receipt-consolidation-preview+support-bundle-manifest-preview+support-case-timeline-preview+snapshot-restore-candidates-preview+state-root-quota-retention-preview",
		...
		NextRequirements: []string{
			...
			"Require the production D-Bus gate review, method review, and consolidated opaque authorization receipt boundary before considering any production ownership claim.",
