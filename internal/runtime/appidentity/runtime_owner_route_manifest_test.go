...
func TestRuntimeOwnerRouteManifestPreviewReportsGoAndLegacyRoutes(t *testing.T) {
	...
	if preview.DesktopSafeSummary != "Runtime owner routes have Go coverage for migrated application catalog, engine catalog, run planning, package acquisition, diagnostics, and current Go previews, with C adapter migration still pending." {
...
func TestRuntimeOwnerRouteManifestPreviewBlocksMissingRouteSources(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "VERSION"), []byte("0.2.193\n"), 0o600); err != nil {
