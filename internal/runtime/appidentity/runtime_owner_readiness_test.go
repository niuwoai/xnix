...
func TestRuntimeOwnerReadinessPreviewAggregatesOwnerGates(t *testing.T) {
	...
	if preview.OwnerProcess.RequestType != "runtime-owner-process-preview" ||
		...
		preview.OwnerProcess.Counts.Passed != 4 ||
		preview.OwnerProcess.Counts.Pending != 1 {
	...
	if preview.OwnerRouteManifest.RequestType != "runtime-owner-route-manifest-preview" ||
		...
		preview.OwnerRouteManifest.Counts.Passed != 6 ||
		preview.OwnerRouteManifest.Counts.Pending != 0 {
	...
	if preview.DesktopSafeSummary != "Runtime owner readiness has full Go read-only route coverage; live production ownership, bus claim, and production-signed recipe trust remain pending." {
