...
func PreviewExternalWinAppApplicationDetail(request ExternalWinAppApplicationDetailRequest) (ExternalWinAppApplicationDetail, error) {
	...
	acceptance, err := loadOptionalQ4StagedExternalWinAppApplicationDetailAcceptance(request.Q4StagedExternalWinAppAcceptancePath)
	if err != nil {
		return ExternalWinAppApplicationDetail{}, err
	}
	if acceptance != nil {
		if err := validateExternalWinAppApplicationDetailAcceptance(bundle, *acceptance); err != nil {
			return ExternalWinAppApplicationDetail{}, err
		}
	}
	...
	detail := ExternalWinAppApplicationDetail{
		...
		Source:                                   source,
		LaunchSourceRequestType:                  strings.TrimSpace(bundle.LaunchSourceRequestType),
		...
		KnownPortableBundleStageLaunchConsumed:   bundle.KnownPortableBundleStageLaunchConsumed,
		KnownPortableBundleStageLaunchVerified:   bundle.KnownPortableBundleStageLaunchVerified,
...
func loadOptionalQ4StagedExternalWinAppApplicationDetailAcceptance(path string) (*Q4StagedExternalWinAppAcceptance, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("read q4 staged external Windows app acceptance: %w", err)
	}
	var acceptance Q4StagedExternalWinAppAcceptance
	if err := json.Unmarshal(content, &acceptance); err != nil {
		return nil, fmt.Errorf("parse q4 staged external Windows app acceptance: %w", err)
	}
...
func validateExternalWinAppApplicationDetail(detail ExternalWinAppApplicationDetail) error {
	...
	case detail.KnownPortableBundleStageLaunchConsumed && (!detail.KnownPortableBundleStageLaunchVerified || detail.LaunchSourceRequestType != "windows-known-app-bundle-stage-and-launch"):
...
func validateExternalWinAppApplicationDetailAcceptance(bundle ExternalWinAppCompatibilityEvidenceBundle, acceptance Q4StagedExternalWinAppAcceptance) error {
	switch {
	case acceptance.SchemaVersion != Q4StagedExternalWinAppAcceptanceSchemaVersion || acceptance.RequestType != Q4StagedExternalWinAppAcceptanceRequestType:
		return errors.New("external Windows app application detail requires a q4 staged external Windows app acceptance payload")
	case !acceptance.AcceptanceReady || !acceptance.SmokeReportConsumed:
		return errors.New("external Windows app application detail requires ready q4 staged external Windows app acceptance")
	...
	case acceptance.HostRootModified || acceptance.PrivilegedContainerRequired || acceptance.HostNetworkingRequired || acceptance.DockerSocketMounted || acceptance.BroadHostMountRequired:
		return errors.New("external Windows app application detail acceptance requires closed host and container gates")
	case acceptance.BackendDetailsExposed || acceptance.RawPathExposed || acceptance.SmokeReportPathExposed || acceptance.RemoteHostExposed || acceptance.DelegatedCommandExposed:
		return errors.New("external Windows app application detail acceptance requires redacted acceptance evidence")
	}
	return nil
}
...
func externalWinAppApplicationDetailSignals(bundle ExternalWinAppCompatibilityEvidenceBundle, fileOpenVerified bool, acceptance *Q4StagedExternalWinAppAcceptance) []ExternalWinAppApplicationDetailSignal {
	...
	if bundle.KnownPortableBundleStageLaunchConsumed {
		signals = append(signals, ExternalWinAppApplicationDetailSignal{ID: "known-portable-bundle-stage-launch", Label: "Known portable bundle launch", Verified: bundle.KnownPortableBundleStageLaunchVerified, Summary: "Runtime verified the known portable bundle staged launch as the direct product evidence source."})
	}
	if acceptance != nil {
...
func externalWinAppApplicationDetailCards(bundle ExternalWinAppCompatibilityEvidenceBundle, fileOpenVerified bool, acceptance *Q4StagedExternalWinAppAcceptance) []ExternalWinAppApplicationDetailReviewCard {
	...
	if bundle.KnownPortableBundleStageLaunchConsumed {
		cards = append(cards, ExternalWinAppApplicationDetailReviewCard{ID: "known-portable-bundle", Title: "Known bundle source", State: boolState(bundle.KnownPortableBundleStageLaunchVerified), Tone: boolTone(bundle.KnownPortableBundleStageLaunchVerified), UserVisible: true, RuntimeOwned: true, KDEPolicyOwner: false, BackendDetailsExposed: false, Summary: "The product detail is backed by the Runtime known portable bundle staged launch path."})
	}
	if acceptance != nil {
