...
> Last updated: 2026-07-13 | Current version: v0.2.56
...
- Runtime product logic is expected to move into C as it becomes core system behavior. The initial C Runtime core now owns stable Runtime identity, reserved write-method enumeration, and write-gate decisions; Ruby remains the test harness and development scaffolding while older models are migrated.
...
- The C Runtime core delegates to `xnix_runtime_core.c` and `xnix-runtime-core`, which expose the stable Runtime D-Bus identity, reserved write methods, and write gate decisions without relying on Ruby for that product boundary.
...
| C Runtime core | C owns stable Runtime identity and write-gate decisions while Ruby remains test tooling |
