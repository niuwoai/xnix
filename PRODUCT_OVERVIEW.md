...
> Last updated: 2026-07-13 | Current version: v0.2.65
...
- Compatibility settings change planning delegates to `xnix-compat-settings-change` and the Runtime `GetCompatibilitySettingsChangePlan` read path, which validate requested user-facing settings values and return confirmation, Portal review, restore-point, and persistence gates without persisting state, granting resources, mutating the host root, or exposing backend details.
