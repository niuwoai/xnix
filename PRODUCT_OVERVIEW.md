...
> Last updated: 2026-07-14 | Current version: v0.2.71
...
- Runtime method parity manifests delegate to `xnix-runtime-method-parity-manifest`, which checks that the XML contract, Runtime dispatch, D-Bus client, smoke adapter, and session smoke agree on all KDE-facing read-only methods while keeping write methods disabled.
...
| Runtime method parity manifest | Runtime verifies read-only method parity across contract, daemon, client, adapter, and smoke coverage |
