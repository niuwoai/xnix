...
> Last updated: 2026-07-13 | Current version: v0.2.58
...
- The C Runtime core delegates to `xnix_runtime_core.c` and `xnix-runtime-core`, which expose the stable Runtime D-Bus identity, reserved write methods, write gate decisions, safe application catalog reads, and engine selection reads without relying on Ruby for that product boundary.
...
| C Runtime core | C owns stable Runtime identity, application catalog metadata, engine selection metadata, and write-gate decisions while Ruby remains test tooling |
