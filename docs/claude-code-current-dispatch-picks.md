...
Use `docs/claude-code-next-work-after-0640-rc1.md` as the current prompt-ready task sheet. Use `docs/xnix-current-mainline.md` as the Codex-owned mainline state and `docs/claude-code-stability-release-train.md` as the cadence authority where it does not conflict with the current goal: one coherent small version per local commit, targeted tests on every small version, and a full build plus complete tests every twenty small versions. The `v0.2.640-rc1` checkpoint candidate is present locally, but the formal `v0.2.640` tag must wait until the full-smoke gate passes.
...
| --- | --- | --- | --- | --- |
| 1 | `C8W1` Full-smoke blocker classifier | `docs/claude-code-next-work-after-0640-rc1.md` | `codex/full-smoke-blocker-classifier` | Make the current Docker Hub EOF blocker and future full-smoke failures reviewable without running Docker, QEMU, or network probes. |
| 2 | `C8W2` Desktop-trigger staged invocation readiness packet | `docs/claude-code-next-work-after-0640-rc1.md` | `codex/desktop-trigger-staged-invocation-readiness` | Prepare the next real desktop-triggered `xnix-compat-launch` smoke while keeping KDE presentation-only and execution disabled. |
| 3 | `C8W3` Runtime owner service launch request envelope guard | `docs/claude-code-next-work-after-0640-rc1.md` | `codex/owner-service-launch-envelope-guard` | Guard future desktop-triggered requests so KDE can only forward opaque evidence handles and cannot reconstruct Runtime launch inputs. |

## Best First Pick

Start with `C8W1` from `docs/claude-code-next-work-after-0640-rc1.md`.

`C8W1` is the safest first branch because it makes the current `v0.2.640-rc1` full-smoke blocker explicit in release evidence without touching Docker, QEMU, network access, Colima state, or host configuration. After that, dispatch `C8W2` and `C8W3` to prepare the real desktop-triggered staged launch lane while preserving Runtime ownership boundaries.
...
```text
You are implementing one bounded Xnix task from docs/claude-code-next-work-after-0640-rc1.md.
