...
Use `docs/claude-code-next-actions-handoff.md` as the short current prompt-ready task sheet and `docs/claude-code-next-work-after-0640-rc1.md` as the detailed eighth-wave reference. Use `docs/xnix-current-mainline.md` as the Codex-owned mainline state and `docs/claude-code-stability-release-train.md` as the cadence authority where it does not conflict with the current goal: one coherent small version per local commit, targeted tests on every small version, and a full build plus complete tests every twenty small versions. The `v0.2.640-rc3` checkpoint candidate is present locally, but the formal `v0.2.640` tag must wait until the full-smoke gate passes.
...
1. `C8W3` from `docs/claude-code-next-actions-handoff.md`: Runtime owner service launch request envelope guard.
2. `C8W4` from `docs/claude-code-next-actions-handoff.md`: KDE controlled-launch action surface audit.
3. `C8W5` from `docs/claude-code-next-actions-handoff.md`: Managed launcher acceptance report.
...
| --- | --- | --- | --- | --- |
| 1 | `C8W2` Desktop-trigger staged invocation readiness packet | `docs/claude-code-next-work-after-0640-rc1.md` | `codex/desktop-trigger-staged-invocation-readiness` | Prepare the next real desktop-triggered `xnix-compat-launch` smoke while keeping KDE presentation-only and execution disabled. |
| 2 | `C8W3` Runtime owner service launch request envelope guard | `docs/claude-code-next-work-after-0640-rc1.md` | `codex/owner-service-launch-envelope-guard` | Guard future desktop-triggered requests so KDE can only forward opaque evidence handles and cannot reconstruct Runtime launch inputs. |
| 3 | `C8W4` KDE controlled-launch action surface audit | `docs/claude-code-next-work-after-0640-rc1.md` | `codex/kde-controlled-launch-action-surface-audit` | Verify KDE controlled-launch action metadata stays presentation-only before real desktop-trigger smoke work begins. |

## Best First Pick

Start with `C8W3` from `docs/claude-code-next-actions-handoff.md`.

`C8W1` is already present locally in v0.2.640-rc2, and `C8W2` is present locally in v0.2.640-rc3. Dispatch `C8W3` next to harden the Runtime owner service launch request envelope before any broader desktop-triggered staged launch work.
