...
> Last updated: 2026-07-18 | Baseline: v0.2.325
...
| --- | --- | --- | --- | --- |
| 1 | Restricted owner smoke execution receipt | `docs/windows-app-compatibility-implementation-brief.md` | `codex/restricted-owner-smoke-receipt` | Consume the v0.2.323 activation preflight into a controlled owner-smoke evidence record without claiming the production bus. |
| 2 | Test-only materialization fan-out to KDE surfaces | `docs/windows-app-compatibility-implementation-brief.md` | `codex/test-launch-materialization-kde-fanout` | Let Compatibility Center, task manager, tray, and notification previews consume the v0.2.324 materialization receipt without enabling launch. |
| 3 | Backend adapter fixture binding audit | `docs/windows-app-compatibility-implementation-brief.md` | `codex/backend-adapter-fixture-binding-audit` | Consume the v0.2.325 no-op adapter contract from fixture-backed readiness views without invoking a backend. |

## Best First Pick

Start with the restricted owner smoke execution receipt.

The v0.2.320 offline and constrained-host gates pass, including the full Buildroot build and QEMU serial smoke. Authorized q3 and q4 composes produced the Fedora 44 container, clean qcow2, persisted KVM graphical-login evidence, and closed host-boundary proof. The v0.2.321 and v0.2.322 matrix evidence can now consume controlled artifact and snapshot receipts, v0.2.323 turns production Runtime service activation into a fail-closed preflight, v0.2.324 materializes only a test-only review plan from restricted launch evidence, and v0.2.325 defines the no-op backend adapter boundary before real Wine/Proton/VM launch paths are allowed. The next useful blocker is a restricted owner smoke receipt that consumes activation preflight evidence without claiming the production bus. Windows application execution remains unproven.
