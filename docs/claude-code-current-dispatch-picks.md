...
> Last updated: 2026-07-18 | Baseline: v0.2.324
...
| --- | --- | --- | --- | --- |
| 1 | Backend adapter no-op contract | `docs/windows-app-compatibility-implementation-brief.md` | `codex/backend-noop-adapter-contract` | Define the first backend adapter boundary with a no-op implementation before any real Wine, Proton, or VM process is allowed to start. |
| 2 | Restricted owner smoke execution receipt | `docs/windows-app-compatibility-implementation-brief.md` | `codex/restricted-owner-smoke-receipt` | Consume the v0.2.323 activation preflight into a controlled owner-smoke evidence record without claiming the production bus. |
| 3 | Test-only materialization fan-out to KDE surfaces | `docs/windows-app-compatibility-implementation-brief.md` | `codex/test-launch-materialization-kde-fanout` | Let Compatibility Center, task manager, tray, and notification previews consume the v0.2.324 materialization receipt without enabling launch. |

## Best First Pick

Start with the backend adapter no-op contract.

The v0.2.320 offline and constrained-host gates pass, including the full Buildroot build and QEMU serial smoke. Authorized q3 and q4 composes produced the Fedora 44 container, clean qcow2, persisted KVM graphical-login evidence, and closed host-boundary proof. The v0.2.321 and v0.2.322 matrix evidence can now consume controlled artifact and snapshot receipts, v0.2.323 turns production Runtime service activation into a fail-closed preflight, and v0.2.324 materializes only a test-only review plan from restricted launch evidence. The next useful blocker is the backend adapter boundary: define the no-op adapter contract before any real Wine/Proton/VM launch path is allowed. Windows application execution remains unproven.
