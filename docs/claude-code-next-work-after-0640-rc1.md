...
`C8W1` is implemented locally in v0.2.640-rc2, `C8W2` is implemented locally in v0.2.640-rc3, `C8W3` is implemented locally in v0.2.640-rc4, and `C8W4` is implemented locally in v0.2.640-rc5. Use this document for `C8W5` through `C8W6` unless a reviewer explicitly asks for a repair branch.
...
The `v0.2.640-rc5` candidate already promotes `kde-controlled-launch-action-dbus-fixture-smoke` into `scripts/full_smoke.rb` as a required PASS lane, adds structured full-smoke failure classification, adds a Go-owned desktop-trigger staged invocation readiness packet, adds a fail-closed owner-service launch envelope guard, and audits the KDE controlled-launch action surface. The restricted container command runs the plan-consuming KDE controlled-launch action harness in the tested Runtime image with explicit D-Bus fixture execution enabled, no container networking, no privileged container, no Docker socket, managed cache volume access, and the controlled-launch scratch tmpfs.
...
| 1 | `C8W5` Managed launcher acceptance report | known Windows app / Compatibility Center | Tie the known 7-Zip guest smoke, launcher bridge, and Center projection into a user-safe acceptance summary. |
| 2 | `C8W6` Post-checkpoint promotion checklist | release train | Give Codex a deterministic checklist for promoting `0.2.640-rc5` to `0.2.640` only after full smoke passes. |

Prefer dispatching `C8W5` next. It improves user-facing acceptance evidence without relying on Docker, QEMU, or the current external registry problem.
...
Create a deterministic promotion checklist that Codex can use after `ruby scripts/full_smoke.rb` passes to promote `0.2.640-rc5` to formal `0.2.640` without accidentally skipping evidence, retaining stale release-blocked wording, or claiming success from a partial smoke.
...
- Version-promotion steps that replace `0.2.640-rc5` with `0.2.640` only after all required PASS markers exist.
...
Target outcome:
- Add a deterministic post-checkpoint promotion checklist for turning 0.2.640-rc5 into 0.2.640 only after full smoke passes.
...
- Promote `0.2.640-rc5` to `0.2.640` without full-smoke PASS evidence.
