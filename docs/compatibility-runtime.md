...
The C Runtime core delegates to `runtime/core/xnix_runtime_core.c`. It exposes the stable Runtime bus name, object path, interface, reserved write methods, and write-gate decisions through `xnix-runtime-core`. This is the first migration point for product logic that must not remain Ruby-only.
...
- The C Runtime core must expose stable Runtime identity and write-gate decisions without enabling backend execution.
