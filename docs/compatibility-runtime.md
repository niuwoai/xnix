...
- The C Runtime core must expose stable Runtime identity, safe application catalog records, compatibility engine catalog records, Portal access policy records, snapshot policy records, application state-root policy records, install readiness policy records, artifact manifest policy records, acquisition preflight policy records, package source policy records, backend binding policy records, settings policy records, settings change policy records, service binding policy records, live owner gate policy records, and write-gate decisions without enabling backend execution, host permission changes, or host-root mutation.
