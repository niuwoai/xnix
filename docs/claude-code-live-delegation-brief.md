...
> Last updated: 2026-07-24 | Mainline checkpoint: v0.2.640-rc4 | Formal release: v0.2.640 pending full smoke
...
No Codex-owned dirty lane should be present after the local v0.2.640-rc4 checkpoint. If these C8W3 files are dirty, treat that as interrupted C8W3 work rather than permission to start a new lane:

```text
C8W3 Runtime owner service launch request envelope guard
```

If those files are dirty, Claude Code must not start `C8W4`, `C8W5`, or `C8W6` in the same checkout.
...
| --- | --- | --- | --- | --- |
| 0 | `C8W3` Runtime owner service launch request envelope guard | `codex/owner-service-launch-envelope-guard` | Yes, only if explicitly assigned to finish C8W3 | Finish the fail-closed evidence-only envelope guard. |
| 1 | `C8W4` KDE controlled-launch action surface audit | `codex/kde-controlled-launch-action-surface-audit` | No | Prove the KDE action surface stays presentation-only. |
| 2 | `C8W5` Managed launcher acceptance report | `codex/managed-launcher-acceptance-report` | No | Tie known-app smoke evidence, launcher bridge evidence, and Compatibility Center projection into a safe acceptance report. |
| 3 | `C8W6` Post-checkpoint promotion checklist | `codex/post-checkpoint-promotion-checklist` | No | Produce a deterministic checklist for promoting `v0.2.640` after full smoke passes. |
| 4 | `C8W7` Desktop-trigger dry-run request review | `codex/desktop-trigger-dry-run-request-review` | No | Add the next dry-run review packet before any real desktop-triggered launch attempt. |
