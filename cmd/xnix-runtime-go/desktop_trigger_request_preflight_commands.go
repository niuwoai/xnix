...
func runDesktopTriggerRequestPreflightPreview(args []string, stdout io.Writer) error {
	flags := flag.NewFlagSet(owner.DesktopTriggerRequestPreflightRequestType, flag.ContinueOnError)
	...
	if flags.NArg() != 0 {
		return errors.New("desktop-trigger-request-preflight-preview does not accept positional arguments")
	}
	if strings.TrimSpace(*stateRoot) == "" {
		return errors.New("desktop-trigger-request-preflight-preview requires --state-root")
	}
	if strings.TrimSpace(*desktopEntryFile) == "" {
		return errors.New("desktop-trigger-request-preflight-preview requires --desktop-entry-file")
	}
	contents, err := os.ReadFile(*desktopEntryFile)
	if err != nil {
		return errors.New("desktop-trigger-request-preflight-preview could not read desktop entry metadata")
	}
	preview, err := owner.PreviewDesktopTriggerRequestPreflight(owner.DesktopTriggerRequestPreflightRequest{
		StateRoot:              *stateRoot,
