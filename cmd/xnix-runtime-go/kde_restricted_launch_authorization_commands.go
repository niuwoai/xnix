...
func runKDERestrictedLaunchAuthorizationRecord(args []string, stdout io.Writer) error {
	flags := flag.NewFlagSet("kde-restricted-launch-authorization-record", flag.ContinueOnError)
	...
	mode := flags.String("mode", "", "required authorization mode; must be test-only")
	...
	if *registryPath == "" || *applicationID == "" || *stateRoot == "" {
		return errors.New("kde-restricted-launch-authorization-record requires --registry, --app, and --state-root")
	}
	if *mode != execution.RestrictedTestMode || *authorize != execution.RestrictedTestPreparationDirective {
		return errors.New("kde-restricted-launch-authorization-record requires --mode test-only and --authorize authorize-restricted-test-preparation")
	}
	if flags.NArg() != 0 {
		return errors.New("kde-restricted-launch-authorization-record does not accept positional arguments")
	}
	recipeRecord, provenance, err := appidentity.LoadRecipeFromRegistry(*registryPath, *recipeRoot, *applicationID)
	if err != nil {
		return err
	}
	record, err := appidentity.NewKDERestrictedLaunchAuthorizationRecord(recipeRecord, provenance, appidentity.KDERestrictedLaunchAuthorizationOptions{StateRoot: *stateRoot, Mode: *mode, Directive: *authorize})
	if err != nil {
		return err
	}
