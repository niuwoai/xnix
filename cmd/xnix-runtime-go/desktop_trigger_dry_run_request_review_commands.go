...
func runDesktopTriggerDryRunRequestReviewPreview(args []string, stdout io.Writer) error {
	flags := flag.NewFlagSet(owner.DesktopTriggerDryRunRequestReviewRequestType, flag.ContinueOnError)
	...
	if flags.NArg() != 0 {
		return errors.New("desktop-trigger-dry-run-request-review-preview does not accept positional arguments")
	}
	if strings.TrimSpace(*stateRoot) == "" {
		return errors.New("desktop-trigger-dry-run-request-review-preview requires --state-root")
	}
	if strings.TrimSpace(*desktopEntryFile) == "" {
		return errors.New("desktop-trigger-dry-run-request-review-preview requires --desktop-entry-file")
	}
	contents, err := os.ReadFile(*desktopEntryFile)
	if err != nil {
		return errors.New("desktop-trigger-dry-run-request-review-preview could not read desktop entry metadata")
	}
	preview, err := owner.PreviewDesktopTriggerDryRunRequestReview(owner.DesktopTriggerDryRunRequestReviewRequest{
		StateRoot:              *stateRoot,
