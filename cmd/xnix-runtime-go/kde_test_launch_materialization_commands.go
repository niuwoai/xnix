...
func runKDETestLaunchMaterializationRecord(args []string, stdout io.Writer) error {
	flags := flag.NewFlagSet("kde-test-launch-materialization-record", flag.ContinueOnError)
	...
	mode := flags.String("mode", "", "required materialization mode; must be test-only")
	...
	if *registryPath == "" || *applicationID == "" || *stateRoot == "" {
		return errors.New("kde-test-launch-materialization-record requires --registry, --app, and --state-root")
	}
	if *mode != execution.RestrictedTestMode || *authorize != execution.RestrictedTestPreparationDirective {
		return errors.New("kde-test-launch-materialization-record requires --mode test-only and --authorize authorize-restricted-test-preparation")
	}
	if flags.NArg() != 0 {
		return errors.New("kde-test-launch-materialization-record does not accept positional arguments")
	}
	recipeRecord, provenance, err := appidentity.LoadRecipeFromRegistry(*registryPath, *recipeRoot, *applicationID)
	if err != nil {
		return err
	}
	record, err := appidentity.NewKDETestLaunchMaterializationRecord(recipeRecord, provenance, appidentity.KDERestrictedLaunchAuthorizationOptions{StateRoot: *stateRoot, Mode: *mode, Directive: *authorize})
	if err != nil {
		return err
	}
