...
func runDesktopTriggerStagedInvocationReadinessPreview(args []string, stdout io.Writer) error {
	flags := flag.NewFlagSet(appidentity.DesktopTriggerStagedInvocationReadinessRequestType, flag.ContinueOnError)
	...
	if flags.NArg() != 0 {
		return errors.New("desktop-trigger-staged-invocation-readiness-preview does not accept positional arguments")
	}
	if strings.TrimSpace(*stateRoot) == "" {
		return errors.New("desktop-trigger-staged-invocation-readiness-preview requires --state-root")
	}
	preview, err := appidentity.PreviewDesktopTriggerStagedInvocationReadiness(appidentity.DesktopTriggerStagedInvocationReadinessRequest{
		StateRoot:              *stateRoot,
