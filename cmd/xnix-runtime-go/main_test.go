...
func TestBackendEnvironmentPreviewCommandRendersEnvironmentPlan(t *testing.T) {
	root := t.TempDir()
	recipeData := []byte(`{"id":"org.example.ledger","name":"Example Ledger","icon":"office-chart-area","mode":"automatic","supported_extensions":[".abc"]}`)
	sum := sha256.Sum256(recipeData)
	digest := hex.EncodeToString(sum[:])
	...
	var output bytes.Buffer
	err := run([]string{"backend-environment-preview", "--registry", registryPath, "--app", "org.example.ledger"}, &output)
	if err != nil {
		t.Fatalf("run returned error: %v", err)
	}
	...
	if payload["schema_version"] != "xnix.runtime.backend_environment.v1" ||
		payload["request_type"] != "backend-environment-preview" ||
		payload["plan_type"] != "compatibility-backend-environment-plan" ||
		payload["runtime_method"] != "GetBackendEnvironmentPlan" {
		t.Fatalf("unexpected backend environment schema: %#v", payload)
	}
	if payload["desktop"] != "KDE Plasma" ||
		payload["application_id"] != "org.example.ledger" ||
		payload["display_name"] != "Example Ledger" ||
		payload["desktop_file"] != "xnix-org.example.ledger.desktop" {
		t.Fatalf("unexpected backend environment identity: %#v", payload)
	}
	if payload["recommended_profile_id"] != "local-compatibility" ||
		payload["environment_state"] != "planned-blocked" ||
		payload["profile_count"] != float64(2) ||
		payload["ready_profile_count"] != float64(0) ||
		payload["blocked_profile_count"] != float64(2) {
		t.Fatalf("unexpected backend environment recommendation: %#v", payload)
	}
	profileIDs := payload["profile_ids"].([]any)
	if profileIDs[0] != "local-compatibility-environment" ||
		profileIDs[1] != "isolated-compatibility-environment" {
		t.Fatalf("unexpected profile ids: %#v", profileIDs)
	}
	profiles := payload["profiles"].([]any)
	localProfile := profiles[0].(map[string]any)
	if localProfile["id"] != "local-compatibility-environment" ||
		localProfile["status"] != "blocked" ||
		localProfile["recommended"] != true ||
		localProfile["environment_created"] != false ||
		localProfile["backend_process_started"] != false ||
		localProfile["storage_path_exposed"] != false ||
		localProfile["backend_details_exposed"] != false {
		t.Fatalf("unexpected local environment profile: %#v", localProfile)
	}
	bridgeIDs := payload["bridge_capability_ids"].([]any)
	expectedBridgeIDs := []string{"file-open", "uri-open", "clipboard", "print", "screenshot"}
	for index, expected := range expectedBridgeIDs {
		if bridgeIDs[index] != expected {
			t.Fatalf("bridge_capability_ids[%d] = %#v, want %q", index, bridgeIDs[index], expected)
		}
	}
	bridges := payload["bridge_capabilities"].([]any)
	for _, value := range bridges {
		bridge := value.(map[string]any)
		if bridge["runtime_method"] != "GetPortalRequestPlan" ||
			bridge["state"] != "planned" ||
			bridge["portal_required"] != true ||
			bridge["user_review_required"] != true ||
			bridge["bridge_enabled"] != false ||
			bridge["request_object_created"] != false ||
			bridge["direct_backend_access_allowed"] != false ||
			bridge["backend_details_exposed"] != false {
			t.Fatalf("bridge gate unexpectedly open: %#v", bridge)
		}
	}
	if payload["runtime_owned"] != true || payload["go_runtime_backed"] != true ||
		payload["kde_policy_owner"] != false || payload["user_visible"] != true ||
		payload["local_environment_ready"] != false ||
		payload["isolated_environment_ready"] != false ||
		payload["environment_created"] != false ||
		payload["backend_process_started"] != false ||
		payload["backend_binding_ready"] != false ||
		payload["launch_enabled"] != false ||
		payload["request_object_created"] != false ||
		payload["host_storage_exposed"] != false ||
		payload["clipboard_bridge_enabled"] != false ||
		payload["print_bridge_enabled"] != false ||
		payload["file_bridge_enabled"] != false ||
		...
		t.Fatalf("unexpected backend environment safety flags: %#v", payload)
	}
}
...
func TestKDEActionQueuePreviewCommandAggregatesEntryActions(t *testing.T) {
	root := t.TempDir()
	recipeData := []byte(`{"id":"org.example.ledger","name":"Example Ledger","icon":"office-chart-area","mode":"automatic","supported_extensions":[".abc"]}`)
	sum := sha256.Sum256(recipeData)
	digest := hex.EncodeToString(sum[:])
	...
	var output bytes.Buffer
	err := run([]string{"kde-action-queue-preview", "--registry", registryPath, "--app", "org.example.ledger", "--decision", "approved", "file:///home/test/Documents/book.abc"}, &output)
	if err != nil {
		t.Fatalf("run returned error: %v", err)
	}
	...
	if payload["schema_version"] != "xnix.runtime.kde_action_queue.v1" ||
		payload["request_type"] != "kde-action-queue-preview" ||
		payload["queue_type"] != "compatibility-center-kde-action-queue" ||
		payload["source"] != "kde-entrypoint-action-preview" ||
		payload["action_count"] != float64(7) ||
		payload["pending_action_count"] != float64(7) ||
		payload["portal_action_count"] != float64(1) ||
		payload["runtime_gate_action_count"] != float64(7) {
		t.Fatalf("unexpected KDE action queue schema or counts: %#v", payload)
	}
	if payload["action_queue_created"] != true ||
		payload["action_queue_persisted"] != false ||
		payload["request_objects_created"] != false ||
		payload["execution_started"] != false ||
		payload["backend_process_started"] != false ||
		payload["host_root_modified"] != false ||
		payload["backend_details_exposed"] != false {
		t.Fatalf("unexpected KDE action queue safety flags: %#v", payload)
	}
}

func TestKDEActionReviewPreviewCommandCapturesQueueDecision(t *testing.T) {
	root := t.TempDir()
	recipeData := []byte(`{"id":"org.example.ledger","name":"Example Ledger","icon":"office-chart-area","mode":"automatic","supported_extensions":[".abc"]}`)
	sum := sha256.Sum256(recipeData)
	digest := hex.EncodeToString(sum[:])
	...
	var output bytes.Buffer
	err := run([]string{"kde-action-review-preview", "--registry", registryPath, "--app", "org.example.ledger", "--action", "review-file-manager-action", "--decision", "approved", "file:///home/test/Documents/book.abc"}, &output)
	if err != nil {
		t.Fatalf("run returned error: %v", err)
	}
	...
	if payload["schema_version"] != "xnix.runtime.kde_action_review.v1" ||
		payload["request_type"] != "kde-action-review-preview" ||
		payload["review_type"] != "compatibility-center-kde-action-review" ||
		payload["source"] != "kde-action-queue-preview" ||
		payload["runtime_method"] != "ReviewKDEAction" ||
		payload["read_method"] != "GetKDEActionReviewPreview" {
		t.Fatalf("unexpected KDE action review schema: %#v", payload)
	}
	action := payload["action"].(map[string]any)
	decision := payload["decision"].(map[string]any)
	if action["id"] != "review-file-manager-action" ||
		...
		payload["execution_started"] != false ||
		payload["host_root_modified"] != false {
		t.Fatalf("unexpected KDE action review payload: %#v", payload)
	}
}
...
func TestKDEActionReceiptPreviewCommandShapesReceiptWithoutRecording(t *testing.T) {
	root := t.TempDir()
	recipeData := []byte(`{"id":"org.example.ledger","name":"Example Ledger","icon":"office-chart-area","mode":"automatic","supported_extensions":[".abc"]}`)
	sum := sha256.Sum256(recipeData)
	digest := hex.EncodeToString(sum[:])
	...
	var output bytes.Buffer
	err := run([]string{"kde-action-receipt-preview", "--registry", registryPath, "--app", "org.example.ledger", "--action", "review-file-manager-action", "--decision", "approved", "file:///home/test/Documents/book.abc"}, &output)
	if err != nil {
		t.Fatalf("run returned error: %v", err)
	}
	...
	if payload["schema_version"] != "xnix.runtime.kde_action_receipt.v1" ||
		payload["request_type"] != "kde-action-receipt-preview" ||
		payload["receipt_type"] != "compatibility-center-kde-action-review-receipt" ||
		payload["receipt_id"] != "compat-review-org.example.ledger-review-file-manager-action-approved" ||
		payload["receipt_state"] != "preview-only" ||
		payload["source"] != "kde-action-preflight-preview" ||
		payload["runtime_method"] != "RecordKDEActionReviewReceipt" ||
		payload["read_method"] != "GetKDEActionReceiptPreview" {
		t.Fatalf("unexpected KDE action receipt schema: %#v", payload)
	}
	action := payload["action"].(map[string]any)
	review := payload["review"].(map[string]any)
	...
	if action["id"] != "review-file-manager-action" ||
		...
		review["decision"] != "approved" ||
		review["decision_recorded"] != false ||
		preflight["request_type"] != "kde-action-preflight-preview" ||
		preflight["receipt_gate_status"] != "pending" ||
		preflight["review_receipt_recorded"] != false ||
		payload["receipt_preview_created"] != true ||
		payload["receipt_recordable"] != true ||
		payload["review_receipt_recorded"] != false ||
		payload["request_objects_created"] != false ||
		payload["execution_started"] != false ||
		payload["host_root_modified"] != false {
		t.Fatalf("unexpected KDE action receipt payload: %#v", payload)
	}
	fields := payload["receipt_fields"].([]any)
	if len(fields) != 5 || payload["field_count"] != float64(5) {
		t.Fatalf("unexpected receipt fields: %#v", payload)
	}
}

func TestKDEActionStatusPreviewCommandShowsWaitingState(t *testing.T) {
	root := t.TempDir()
	recipeData := []byte(`{"id":"org.example.ledger","name":"Example Ledger","icon":"office-chart-area","mode":"automatic","supported_extensions":[".abc"]}`)
	sum := sha256.Sum256(recipeData)
	digest := hex.EncodeToString(sum[:])
	...
	var output bytes.Buffer
	err := run([]string{"kde-action-status-preview", "--registry", registryPath, "--app", "org.example.ledger", "--action", "review-file-manager-action", "--decision", "approved", "file:///home/test/Documents/book.abc"}, &output)
	if err != nil {
		t.Fatalf("run returned error: %v", err)
	}
	...
	if payload["schema_version"] != "xnix.runtime.kde_action_status.v1" ||
		payload["request_type"] != "kde-action-status-preview" ||
		payload["status_type"] != "compatibility-center-kde-action-status" ||
		payload["status_state"] != "waiting-for-runtime-gates" ||
		payload["source"] != "kde-action-receipt-preview" ||
		payload["runtime_method"] != "GetKDEActionStatus" ||
		payload["read_method"] != "GetKDEActionStatusPreview" {
		t.Fatalf("unexpected KDE action status schema: %#v", payload)
	}
	receipt := payload["receipt"].(map[string]any)
	userVisibleState := payload["user_visible_state"].(map[string]any)
	if receipt["request_type"] != "kde-action-receipt-preview" ||
		receipt["decision"] != "approved" ||
		receipt["review_receipt_recorded"] != false ||
		userVisibleState["badge"] != "Waiting" ||
		userVisibleState["compatibility_center_status"] != "waiting-for-runtime-gates" ||
		payload["status_preview_created"] != true ||
		payload["status_persisted"] != false ||
		payload["review_receipt_recorded"] != false ||
		payload["request_objects_created"] != false ||
		payload["execution_started"] != false ||
		payload["host_root_modified"] != false {
		t.Fatalf("unexpected KDE action status payload: %#v", payload)
	}
}
...
func TestNotificationPreviewCommandConsumesActivationRoot(t *testing.T) {
	root := t.TempDir()
	recipeData := []byte(`{"id":"org.example.ledger","name":"Example Ledger","icon":"office-chart-area","mode":"automatic","supported_extensions":[".abc",".xls"]}`)
	sum := sha256.Sum256(recipeData)
	digest := hex.EncodeToString(sum[:])
	...
	var output bytes.Buffer
	err := run([]string{"notification-preview", "--registry", registryPath, "--app", "org.example.ledger", "--event", "approval-required", "--activation-root", stageRoot}, &output)
	if err != nil {
		t.Fatalf("run returned error: %v", err)
	}
	...
	if payload["activation_receipt_root"] != true ||
		payload["activation_receipt_backed"] != true ||
		payload["activation_receipt_path"] != "usr/share/xnix/compatibility/activation-receipts/org.example.ledger.json" {
		t.Fatalf("unexpected receipt-backed notification payload: %#v", payload)
	}
	if payload["action_execution_enabled"] != false ||
		payload["repair_execution_enabled"] != false ||
		payload["settings_persistence_enabled"] != false ||
		payload["host_root_modified"] != false ||
		payload["backend_details_exposed"] != false {
		t.Fatalf("receipt-backed notification preview must remain gated: %#v", payload)
	}
	if strings.Contains(output.String(), stageRoot) {
		t.Fatalf("notification preview exposed activation root: %s", output.String())
	}
}
...
func TestSettingsPreviewCommandConsumesActivationRoot(t *testing.T) {
	root := t.TempDir()
	recipeData := []byte(`{"id":"org.example.ledger","name":"Example Ledger","icon":"office-chart-area","mode":"automatic","supported_extensions":[".abc"]}`)
	sum := sha256.Sum256(recipeData)
	digest := hex.EncodeToString(sum[:])
	...
	var output bytes.Buffer
	err := run([]string{"settings-preview", "--registry", registryPath, "--app", "org.example.ledger", "--activation-root", stageRoot}, &output)
	if err != nil {
		t.Fatalf("run returned error: %v", err)
	}
	...
	if payload["activation_receipt_root"] != true ||
		payload["activation_receipt_backed"] != true ||
		payload["activation_receipt_path"] != "usr/share/xnix/compatibility/activation-receipts/org.example.ledger.json" {
		t.Fatalf("settings preview did not consume activation receipt: %#v", payload)
	}
	if payload["settings_persisted"] != false ||
		payload["settings_persistence_enabled"] != false ||
		payload["host_root_modified"] != false ||
		payload["backend_details_exposed"] != false {
		t.Fatalf("receipt-backed settings preview must remain gated: %#v", payload)
	}
	if strings.Contains(output.String(), stageRoot) {
		t.Fatalf("settings preview exposed activation root: %s", output.String())
	}
}
...
func TestTrayStatusPreviewCommandConsumesActivationRoot(t *testing.T) {
	root := t.TempDir()
	recipeData := []byte(`{"id":"org.example.ledger","name":"Example Ledger","icon":"office-chart-area","mode":"automatic","supported_extensions":[".abc",".xls"]}`)
	sum := sha256.Sum256(recipeData)
	digest := hex.EncodeToString(sum[:])
	...
	var output bytes.Buffer
	err := run([]string{"tray-status-preview", "--registry", registryPath, "--app", "org.example.ledger", "--activation-root", stageRoot}, &output)
	if err != nil {
		t.Fatalf("run returned error: %v", err)
	}
	...
	if payload["activation_receipt_root"] != true ||
		payload["activation_receipt_backed"] != true ||
		payload["activation_receipt_path"] != "usr/share/xnix/compatibility/activation-receipts/org.example.ledger.json" {
		t.Fatalf("unexpected receipt-backed tray payload: %#v", payload)
	}
	if payload["live_backend_bridge_enabled"] != false ||
		payload["bridge_configuration_persisted"] != false ||
		payload["host_root_modified"] != false ||
		payload["backend_details_exposed"] != false {
		t.Fatalf("receipt-backed tray preview must remain gated: %#v", payload)
	}
	if strings.Contains(output.String(), stageRoot) {
		t.Fatalf("tray preview exposed activation root: %s", output.String())
	}
}
