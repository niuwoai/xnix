...
%w[XnixRuntimeApplication xnix_runtime_application_count xnix_runtime_find_application].each do |token|
  assert(c_runtime_core_header.include?(token), "C Runtime core header must include #{token}")
end
%w[xnix_runtime_write_gate blocked-until-production-backend InstallRecipe Launch CreateSnapshot RestoreSnapshot xnix_runtime_find_application org.xnix.sample.notepad application/x-xnix-txt].each do |token|
  assert(c_runtime_core_source.include?(token), "C Runtime core source must include #{token}")
end
%w[business_logic_runtime tests-and-development-tools application_catalog_owner list-applications get-application write-gate].each do |token|
