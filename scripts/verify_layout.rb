...
%w[Xnix\ Current\ Mainline Codex-owned\ current\ mainline Product\ Goal Ownership\ Rules First-Release\ KDE\ Entrypoints Current\ Safe\ Next\ Task Safety\ Gates KDE\ Plasma\ is\ the\ only\ flagship\ desktop Runtime\ business\ logic\ is\ Go-first Ruby\ is\ for\ tests C\ remains\ for\ low-level redacted\ status\ writer\ authorization\ gate\ audit redacted\ status\ writer\ authorization\ receipt\ audit redacted\ status\ writer\ authorization\ receipt\ acceptance\ audit redacted\ status\ writer\ accepted\ receipt\ gate\ audit].each do |token|
