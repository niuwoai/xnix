...
def assert_owner_trigger_response(stdout, evidence_record, desktop_trigger)
  payload = variant_string_field(stdout, "go_owner_trigger_json")
  trigger = JSON.parse(payload)
  assert(trigger["schema_version"] == "xnix.runtime.known_app_runtime_status_launch_owner_trigger.v1", "D-Bus owner trigger must expose the Go trigger schema")
  assert(trigger["request_type"] == "known-app-runtime-status-launch-owner-trigger-preview", "D-Bus owner trigger must expose the Go trigger request type")
  assert(trigger["runtime_method"] == "PreviewKnownAppRuntimeStatusLaunchOwnerTrigger", "D-Bus owner trigger must preserve the Go preview runtime method")
  assert(trigger["read_method"] == "GetKnownAppRuntimeStatusLaunchOwnerTrigger", "D-Bus owner trigger must preserve the Go preview read method")
  assert(trigger["desktop_dbus_method"] == desktop_trigger.fetch("method"), "D-Bus owner trigger must preserve the public D-Bus method")
  assert(trigger["owner_service_call_args"] == [desktop_trigger.fetch("owner_service_method"), desktop_trigger.fetch("handoff_kind"), desktop_trigger.fetch("evidence_relative_path")], "D-Bus owner trigger must provide the owner service call args consumed by the C adapter")
  assert(trigger["evidence_relative_path"] == evidence_record.fetch("evidence_relative_path"), "D-Bus owner trigger must preserve the relative evidence path")
  assert(trigger["evidence_sha256"] == evidence_record.fetch("evidence_sha256"), "D-Bus owner trigger must preserve the evidence digest")
  assert(trigger["evidence_handoff_consumed"] == true, "D-Bus owner trigger must consume the handoff")
  assert(trigger["evidence_digest_verified"] == true, "D-Bus owner trigger must verify the handoff digest")
  assert(trigger["runtime_owner_service_supplies_inputs"] == true, "D-Bus owner trigger must keep owner inputs supplied by Runtime")
  assert(trigger["kde_forwards_only_evidence_handle"] == true, "D-Bus owner trigger must keep KDE evidence-only")
  assert(trigger["desktop_receipt_fields_reconstructed"] == false, "D-Bus owner trigger must keep receipt reconstruction disabled")
  assert(trigger["desktop_kde_state_root_access"] == false, "D-Bus owner trigger must keep KDE state-root access disabled")
  assert(trigger["state_root_path_exposed"] == false, "D-Bus owner trigger must not expose state root paths")
  assert(trigger["raw_launcher_output_exposed"] == false, "D-Bus owner trigger must not expose raw launcher output")
  assert(trigger["backend_details_exposed"] == false, "D-Bus owner trigger must not expose backend details")
  assert(trigger["host_root_modified"] == false, "D-Bus owner trigger must not mutate the host root")
end
...
begin
  ...
  assert(stdout.include?("go_owner_trigger_available"), "D-Bus response must expose Go owner trigger availability")
  assert(stdout.include?("go_owner_trigger_json"), "D-Bus response must expose Go owner trigger metadata")
  ...
  assert_owner_trigger_response(stdout, evidence_record, desktop_trigger)
