...
DOMAIN_DEFINITIONS = [
  {
    ...
    gate_tokens: {
      "cmd/xnix-runtime-owner/main.go" => %w[smoke-owner deny-write materialization-fanout-owner-smoke-coverage restricted-smoke-fanout-owner-smoke-coverage NewKDETestLaunchMaterializationFanOutOwnerSmokeCoveragePreview NewRestrictedOwnerSmokeReceiptFanOutOwnerSmokeCoveragePreview],
    ...
    summary: "The Go owner candidate has an in-process service-call boundary, private session-bus smoke transcript evidence, D-Bus service-call bridge evidence, full D-Bus read dispatch coverage, lifecycle JSONL, service-call smoke-batch read/write evidence, restricted owner smoke execution receipts under explicit state roots, read-only receipt fan-out to Runtime readiness and support surfaces, explicit materialization fan-out owner smoke coverage, explicit restricted owner smoke fan-out smoke coverage, and C D-Bus bridge evidence for owner self-description, Runtime owner readiness payloads, foundation catalog/status payloads, KDE shell, desktop activation payloads, seven-entry-point payloads, second-ring KDE resource payloads, install-input payloads, backend lifecycle payloads, execution readiness payloads, AI safety payloads, settings/review payloads, Compatibility Center action payloads, and KDE Compatibility Center page payloads; production ownership remains gated."
  },
  {
  ...
  },
  {
    ...
    gate_tokens: {
      ...
      "internal/runtime/appidentity/kde_test_launch_materialization_owner_route_audit.go" => %w[xnix.runtime.kde_test_launch_materialization_owner_route_audit.v1 kde-test-launch-materialization-owner-route-audit-preview NewKDETestLaunchMaterializationOwnerRouteAuditPreview GetKDETestLaunchMaterializationOwnerRouteAudit owner-local-route-ready owner-local-read-route-ready-production-dbus-blocked materialization-fanout-owner-smoke-coverage owner-route-present production-dbus-absent read-only-consume-registered caller-path-boundary receipt-creation-split owner-managed-opaque-lookup route-decision OwnerDispatchRoutePresent ProductionDBusMethodPresent MaterializationWritesStateRoot ReadOnlyReceiptConsumptionReady OwnerManagedOpaqueReceiptLookupReady OpaqueMaterializationReceiptIDSupported OwnerLocalRouteCandidateReady ProductionDBusExposureReady BackendLaunchEnabled HostRootModified],
      "internal/runtime/appidentity/kde_test_launch_materialization_owner_route_audit_test.go" => %w[TestKDETestLaunchMaterializationOwnerRouteAuditSeesConsumeSplit TestKDETestLaunchMaterializationOwnerRouteAuditFailsClosedWithoutSources owner-local-route-ready materialization-owner-route-sources-missing caller-path-boundary owner-managed-opaque-lookup],
