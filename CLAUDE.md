...
- Use q4 for repeated or heavy compilation so local development remains responsive; do not move secrets, host-only private material, or broad host directories to q4.
...
ruby scripts/q4_known_portable_winapp_run.rb
...
## Remote-First Build Workflow

- Use `ruby scripts/remote_go_build.rb` without `--execute` to review the q4 build plan.
- Use `ruby scripts/remote_go_build.rb --execute` for Go command builds that would otherwise run on the macOS host.
- Use `ruby scripts/remote_go_test.rb --execute` for targeted Go test runs that require compilation.
- Use `ruby scripts/q4_full_smoke.rb --execute` only for explicit full-smoke gates or human-approved heavy validation.
- Keep local checks to Ruby layout verification, diff checks, static text searches, and dry-run JSON plan output unless a human explicitly approves a local restricted smoke.
- Every new small version uses targeted validation only; run the full gate every 20 code versions or when a human explicitly authorizes it.
