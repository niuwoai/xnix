...
The project is currently at `v0.2.640-rc46`.
...
v0.2.640-rc46 maps parsed PE architecture into Wine prefix policy, reporting `wine_architecture` and using `win64` for x86_64 apps and `win32` for x86 apps. The previous v0.2.640-rc45 checkpoint added PE machine architecture parsing before runner execution.
...
Run full Buildroot/QEMU smoke at the configured checkpoint cadence. The current formal full checkpoint candidate is `v0.2.640-rc46`; promote to `v0.2.640` only after `ruby scripts/full_smoke.rb` passes and the promotion packet allows the promotion.
