...
The project is currently at `v0.2.640-rc43`.
...
v0.2.640-rc43 makes profile preflight verify the executable has a Windows `MZ` signature before reporting readiness, catching wrong-file profiles before any app launch. The previous v0.2.640-rc42 checkpoint wired Go-owned preflight into `scripts/winapp_smoke.rb` and added `--preflight-only`.
...
Run full Buildroot/QEMU smoke at the configured checkpoint cadence. The current formal full checkpoint candidate is `v0.2.640-rc43`; promote to `v0.2.640` only after `ruby scripts/full_smoke.rb` passes and the promotion packet allows the promotion.
