...
The project is currently at `v0.2.640-rc38`.
...
v0.2.640-rc38 makes local Windows app smoke run from the executable directory by default and adds `--working-dir` for operator-supplied sidecar/resource directories while reports expose only `working_directory_mode`. The previous v0.2.640-rc37 checkpoint added startup-window success mode for GUI-style apps.
...
Run full Buildroot/QEMU smoke at the configured checkpoint cadence. The current formal full checkpoint candidate is `v0.2.640-rc38`; promote to `v0.2.640` only after `ruby scripts/full_smoke.rb` passes and the promotion packet allows the promotion.
