...
The project is currently at `v0.2.43`. KDE Plasma is the only supported flagship desktop. An independent Compatibility Runtime manages application recipes, Wine/VM backends, managed package source selection, managed application state roots, managed backend binding status, diagnostics, AI diagnostic inputs and recommendations, AI repair approval gates, Runtime service binding status, compatibility tests, snapshots, and rollback without exposing implementation details in normal desktop entry points.
