...
The project is currently at `v0.2.640-rc84`.
...
v0.2.640-rc84 makes the q4 real Windows GUI smoke loop practical for daily iteration by defaulting `scripts/remote_wine_guest_gui_smoke.rb` to Runtime-only source sync. The remote harness now syncs only `VERSION`, `go.mod`, `cmd`, `internal`, `runtime`, `scripts`, and `lib` unless `--source-sync-mode full` is requested, keeping the actual Xvfb/QEMU/Wine run and automatic evidence projection intact without copying the full checkout.
