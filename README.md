...
The project is currently at `v0.2.618`.
...
v0.2.618 records the Runtime-status launch evidence projection as a Runtime-owned KDE-readable handoff artifact. The staged 7zr QEMU/Wine dispatch smoke now calls `known-app-kde-runtime-status-launch-evidence-record` after the Runtime wrapper returns `compatibility_center_known_app_evidence`, persists the projection under the Runtime state root, and verifies only relative evidence path plus digest are exposed.
...
```text
GOCACHE=/Users/rocky/Sites/xnix/.cache/go-build go test ./internal/runtime/appidentity -run 'TestRecordKnownAppKDERuntimeStatusLaunchEvidencePersistsSafeHandoff|TestProjectKnownAppKDERuntimeStatusLaunchDelegatedEvidenceBuildsCenterPayload|TestPrepareKnownAppKDERuntimeStatusLaunchExecution' -count=1
GOCACHE=/Users/rocky/Sites/xnix/.cache/go-build go test ./cmd/xnix-runtime-go -run 'TestKnownAppKDERuntimeStatusLaunchEvidenceRecordCommandPersistsProjectionFile|TestKnownAppKDERuntimeStatusLaunchExecutionCommandInvokesManagedLauncherWithRuntimeStateRoot|TestCompatibilityCenterPreviewCommandAcceptsRuntimeProjectedKnownAppEvidenceJSON|TestKDECenterPagePreviewCommandAcceptsRuntimeProjectedKnownAppEvidenceFile' -count=1
