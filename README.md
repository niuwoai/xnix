...
The project is currently at `v0.2.1`. KDE Plasma is the only supported flagship desktop. An independent Compatibility Runtime manages application recipes, Wine/VM backends, diagnostics, snapshots, and rollback without exposing implementation details in normal desktop entry points.
...
The v0.2.1 Runtime foundation defines application recipes, standard desktop entries, a D-Bus service contract, a hardened systemd unit, a Plasma 6 Compatibility Center package skeleton, and a local daemon core that can probe itself, list managed recipes, and report diagnostics. It does not yet ship a full KDE image, real D-Bus method handling, Wine backend, VM backend, or AI service.
